loadstring(game:HttpGet("https://raw.githubusercontent.com/nvb201112/fix-lag/refs/heads/main/fix%20lag%202.luau"))()
local L_1_ = {}
L_1_[3] = table["concat"]
...
if not getgenv()["Configs"] then
    if getgenv().Config then
        ...
        -- Thêm sword nếu bật All Sword hoặc từng cái
        ...
        -- Thêm gun nếu bật All Gun hoặc từng cái
        ...
        -- Config mặc định nếu không có gì
        getgenv()["Configs"] = {
...
game.StarterGui:SetCore("SendNotification", {
    Title = "Kaitun Script",
    Text = "Loading...",
    Icon = "rbxassetid://111131827835431",
    Duration = 10,
})
...
logoBtn.Image = "rbxassetid://111131827835431"
...
logoStroke.Color = Color3.fromRGB(120, 50, 200)
...
main.Size = UDim2.new(0, 430, 0, 290)
...
bg.Image = "rbxassetid://74547655005671"
...
overlay.Parent = main
Instance.new("UICorner", overlay).CornerRadius = UDim.new(0,22)
...
sub.Text = "Kaitun Script Làm Từ A Đến Á"
...
info.Size = UDim2.new(1,-36,0,180)
...
local function createLabel(y, color)
    ...
    l.TextSize = 16
    l.TextColor3 = color or Color3.fromRGB(235,248,255)
    l.TextXAlignment = Enum.TextXAlignment.Left
    l.Parent = info
    Instance.new("UIStroke", l).Color = Color3.fromRGB(80,170,255)
    return l
end

local statusL = createLabel(0, Color3.fromRGB(255,255,255))
local levelL  = createLabel(24, Color3.fromRGB(35,255,226))
local beliL   = createLabel(48, Color3.fromRGB(13,255,0))
local fragL   = createLabel(72, Color3.fromRGB(143,125,255))
local dfL     = createLabel(96, Color3.fromRGB(255,81,247))
local timeL   = createLabel(120, Color3.fromRGB(255,100,100))
local fpsL    = createLabel(144)
local pingL   = createLabel(166)
...
RunService.RenderStepped:Connect(function()
    ...
    fpsL.Text = "FPS: " .. fps
    pingL.Text = "Ping: " .. (pcall(function() return Stats.Network.ServerStatsItem["Data Ping"]:GetValueString() end) and Stats.Network.ServerStatsItem["Data Ping"]:GetValueString() or "N/A")
end)

local function updateUI()
    pcall(function()
        statusL.Text = "Status: Farm"
        levelL.Text = "Level: " .. (player.Data.Level.Value or 0)

        local beli = player.Data.Beli.Value
        if beli >= 1e9 then beliL.Text = string.format("Beli: %.2fB", beli/1e9)
        elseif beli >= 1e6 then beliL.Text = string.format("Beli: %.2fM", beli/1e6)
        elseif beli >= 1e3 then beliL.Text = string.format("Beli: %.1fK", beli/1e3)
        else beliL.Text = "Beli: " .. beli end

        fragL.Text = "Fragments: " .. (player.Data.Fragments.Value or 0)
        local df = player.Data.DevilFruit.Value
        dfL.Text = "Devil Fruits: " .. (df == "" and "None" or df)
        timeL.Text = "Time: " .. formatTime(math.floor(workspace.DistributedGameTime + 0.5))
    end)
end
...
-- Ghi đè Status cho Kaitun
L_1_[45]["Status"] = function(newStatus)
    if statusL then
        statusL.Text = "Status: " .. tostring(newStatus)
    end
end
...
print("✅  UI đã được load!")

-- Các hàm cập nhật
local function updateLevel()
    if L_1_[5].Data.Level then
        LevelLabel.Text = "Level: " .. L_1_[5].Data.Level.Value
    end
end

local function updateBeli()
    if L_1_[5].Data.Beli then
        local beliVal = L_1_[5].Data.Beli.Value
        if beliVal >= 1000000 then
            BeliLabel.Text = "Beli: " .. string.format("%.1fM", beliVal / 1000000)
        elseif beliVal >= 1000 then
            BeliLabel.Text = "Beli: " .. string.format("%.1fK", beliVal / 1000)
        else
            BeliLabel.Text = "Beli: " .. beliVal
        end
    end
end

local function updateFragments()
    if L_1_[5].Data.Fragments then
        FragmentsLabel.Text = "Fragments: " .. L_1_[5].Data.Fragments.Value
    end
end

local function updateDevilFruits()
    if L_1_[5].Data.DevilFruit then
        local dfValue = L_1_[5].Data.DevilFruit.Value
        if dfValue == "" or dfValue == "None" then
            DevilFruitsLabel.Text = "Devil Fruits: None"
        else
            DevilFruitsLabel.Text = "Devil Fruits: " .. dfValue
        end
    end
end
...
local function updateTime()
    ...
    TimeLabel.Text = "Time: " .. formatTime(gameTime)
end

-- Kết nối sự kiện cập nhật
if L_1_[5].Data.Level then
    L_1_[5].Data.Level:GetPropertyChangedSignal("Value"):Connect(updateLevel)
end

if L_1_[5].Data.Beli then
    L_1_[5].Data.Beli:GetPropertyChangedSignal("Value"):Connect(updateBeli)
end

if L_1_[5].Data.Fragments then
    L_1_[5].Data.Fragments:GetPropertyChangedSignal("Value"):Connect(updateFragments)
end

if L_1_[5].Data.DevilFruit then
    L_1_[5].Data.DevilFruit:GetPropertyChangedSignal("Value"):Connect(updateDevilFruits)
end
...
-- Ghi đè hàm Status để tương thích với script cũ
local oldStatusFunc = L_1_[45]["Status"]
L_1_[45]["Status"] = function(newStatus)
    if L_1_[26] then
        L_1_[26]["Text"] = "Status: " .. tostring(newStatus)
    end
    if oldStatusFunc then
        oldStatusFunc(newStatus)
    end
end
